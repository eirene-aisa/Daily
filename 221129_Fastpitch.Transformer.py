...
class PositionalEmbedding(nn.Module):
    def __init__(self, demb):
        ...
        # # encoder/decoder 모두 symbols_embedding_dim이 들어온다.
        self.demb = demb 
        inv_freq = 1 / (10000 ** (torch.arage(0.0, demb, 2.0) / demb))
        # # register_buffer - state_dict에는 저장되지만 최적화 연산은 진행하지 않는 변수
        self.register_buffer('inv+freq', inv_freq)
    
    def forward(self, pos_seq, bsz=None):
        # # pos_seq - input sequence 수 만큼의 배열
...
class PositionwiseConvFF(nn.Module):
    def __init__(self, d_model, d_inner, kernel_size, dropout, pre_lnorm=False):
        ...
        # # d_inner - in_fft_conv1d_filter_size / out_fft_conv1d_filter_size
        # # kernel_size - in_fft_conv1d_kernel_size / out_fft_conv1d_kernel_size
...
class MultiHeadAttn(nn.Module):
    ...
    def _forward(self, inp, attn_mask=None):
        ...
        k = head_q.permute(0, 2, 1, 3).reshape(-1, inp.size(1), d_head)
        v = head_q.permute(0, 2, 1, 3).reshape(-1, inp.size(1), d_head)

        attn_score = torch.bmm(q, k.transpose(1, 2)) # (B, n, m) * (B, m, p) => (B, n, p)
        attn_score.mul_(self.scale)  # 기존 배열에 곱셈 결과가 덧씌워진다

        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).to(attn_score.dtype)
            attn_mask = attn_mask.unsqueeze(1)
            atten_score.masked_fill_(attn_mask.to(torch.bool), -float('inf'))
        
        attn_prob = F.softmax(attn_score, dim=2)
        attn_prob = self.dropatt(attn_prob)
        attn_vec = torch.bmm(attn_prob, v)

        attn_vec = atten_vvec.view(n_head, inp.size(0), inp.size(1), d_head)
...
class TransformerLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, kernel_size, 
                 dropout, **kwargs):
        ...
        for _ in range(n_layer):
            self.layers.append(
                TransformerLayer(
                    ...
                    dropatt=dropattt, pre_lnorm=pre_lnorm
                )
            )
