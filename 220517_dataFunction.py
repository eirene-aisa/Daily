...
class BetaBinomialInterpolator:
    """Interpolates alignment prior matrices to save computation.
       Cache popular sizes and use img interpolation to get priors faster.
    """
...
def estimate_pitch(wav, mel_len, method='pyin', normalize_mean=None, 
    ...
    if type(normalize_mean) is float or type(normalize_mean) is list:
        nomarlize_mean = torch.tensor(normalize_mean)
    ...
    if method == 'pyin':
        ...
        pitch_mel, voiced_flag, voice_probs = librosa.pyin(snd, 
...
class TTSDataset(torch.utils.data.Dataset):
    """ loads (audio, text). normalizes text and converts them to ne-hot sequence
        computes mel-spectrograms from audio files. """

    def __init__(self, dataset_path, audiopaths_and_text, 
                 text_cleaners, n_mel_channels, symbol_set='english_basic', 
                 p_arpabet=1.0, n_speakers=1, 
                 load_mel_from_disk=True, load_pitch_from_disk=True, 
                 pitch_mean=214.72203, pitch_s=65.72038,   # LJSpeech
                 max_wav_value=None, min_wav_value=None, 
                 sampling_rate=None, filter_length=None, hop_length=None, 
                 win_length=None, mel_fmin=None, mel_fmax=None, 
                 prepend_space_to_text=False, append_space_to_text=False, 
                 pitch_online_dir=None, betabinomial_online_dir=None, 
        ...
        self.audiopaths_and_text = load_filepaths_and_text(dataset_path, 
                                                           audiopaths_and_text,
        ...
        if not load_mel_from_disk:
            ...
            self.stft = layers.TacotronSTFT(filter_lengths, hop_length, 
        ...
        assert p_arpabet = 0.0 or p_arpabet == 1.0, (
            ...
            'Variable probabilty breaks caching of betabinomial matrices.')
        ...
        # 원래 if 였음 
        elif len(self.audiopaths_and_text[0]) > expected_columns:
    ...
    def __getitem__(self, index):
        ...
        mel = self.get_mel(audioapth)
        text = self.get_text(text)
        pitch = self.get_pitch(index, mel.size(-1))
        energy = torch.norm(mel.float(), dim=0, p=2)
        attn_prior = self.get_prior(index, mel.shape[1], text.shape)

        assert pitch.size(-1) == mel.size(-1)

        # No higher formants? 이게 무슨 뜻이지 
        if len(pitch.size()) == 1:
            pitch = pitch[None, :]
    ...
    def get_mel(self, filename):
        if not self.load_mel_from_disk:
            ...
            if sampling_rate != self.stft.sampling_rate:
                raise ValueError(f'{sampling_rate} SR does not match target \
                                   {self.stft.sampling_rate} SR')
        
            audio_norm = audio / self.max_wav_value
            audio_norm = audio_norm.unsqueeze(0)
            # TODO : 왜 False 일까? 멜 값이 변하기는 하는데 그거에 따른 loss 는 필요가 없어서 그런가 
            audio_norm = torch.autograd.Variable(audio_norm, requires_grad=False)

            melspec = self.stft.mel_spectrogram(audio_norm)
            melspec = torch.squeeze(melspec, 0)  # 그냥 squeeze 랑 다른가? TODO 

        else:
            melspec = torch.load(filename)
            # assert melspec.size(0) == self.stft.n_mel_channels, ('Mel dimension mismatch')
        
        return melspec

    def get_text(self, text):
        text = self.tp.text_to_sequence(text)
        space = [self.tp.text_to_sequence("A A")[1]]
        ...
        return torch.LongTensor(list(text))
...
class TTSCollate:
    ...
    def __call__(self, batch):
        ...
        mel_padded.zero_()
        output_lengths = torch.LongTensort(len(batch))
        for i in range(len(ids_sorted_decreasing)):
        ...
        return (text_padded, input_lengths, mel_padded, outptu_lengths, len_x, 
                pitch, padded, energy_padded, speaker, atten_prior_padded, audiopaths)
